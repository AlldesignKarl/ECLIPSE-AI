...
const puertoRedis = `${AQUI}redis-web.txt`;
...
const salida = openSync(`${AQUI}app-web.log`, "w");
...
for (let i = 0; i < 240 && !enPie; i++) {
  try { const r = await fetch(`${BASE}/`); enPie = r.ok; } catch { /* todavía no */ }
...
try {
  ...
  await chico.goto(`${BASE}/`, { waitUntil: "networkidle" });
  await esperar(800);
  ...
  const trampa = await fetch(`${BASE}/api/contacto`, {
    method: "POST",
  ...
  const malo = await fetch(`${BASE}/api/contacto`, {
    method: "POST",
    headers: { "content-type": "application/json" },
  ...
  const seguido = await fetch(`${BASE}/api/contacto`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
