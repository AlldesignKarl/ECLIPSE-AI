 ...
 *
 *   1. `WEBHOOK_SOLICITUDES` — un POST con el JSON entero. Es lo que piden
 *      Zapier, Make, n8n, Formspree o un CRM: se pega la dirección y ya está.
 *   2. `RESEND_API_KEY` + `EMAIL_DESTINO` — un correo de verdad.
 *      (`EMAIL_REMITENTE` si el dominio verificado no es el de destino.)
 *   3. La base de datos (Redis por REST, Upstash o el de Vercel). Si está,
...
import { get, set, hayBaseDeDatos, tomarTurno } from "./almacen";
...
async function porWebhook(s: Solicitud, id: string): Promise<boolean> {
  const url = process.env.WEBHOOK_SOLICITUDES;
  if (!url) return false;
...
export function hayDondeEntregar(): boolean {
  return Boolean(
    process.env.WEBHOOK_SOLICITUDES ||
      (process.env.RESEND_API_KEY && process.env.EMAIL_DESTINO) ||
      hayBaseDeDatos(),
  );
}
...
export async function puedeEnviar(huella: string): Promise<boolean> {
  ...
  try {
    return await tomarTurno(`ak:turno:${huella}`, 30);
  } catch {
    return true;
  }
}
